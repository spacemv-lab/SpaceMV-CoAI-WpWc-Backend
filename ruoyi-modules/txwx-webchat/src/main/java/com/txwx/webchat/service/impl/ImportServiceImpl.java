...
public class ImportServiceImpl {
    ...
    public <T extends IImportBaseModel> ImportResultVo importExcel(
            ...
            Consumer<T> validator) throws Exception {
        ...
        // 2. 处理结果：如果有错误，触发下载
        if (!errors.isEmpty()) {
            exportErrorList(response, clazz, errors);
            // 返回错误统计
    ...
    private <T extends IImportBaseModel> void exportErrorList(
            HttpServletResponse response,
            Class<T> clazz,
            List<RowErrorVo> errors) throws IOException {
        response.reset();
        // 1. 设置响应头
        ...
        for (RowErrorVo error : errors) {
            ...
            if (rowData != null) {
                ...
                Object[] originArray = rowData.toObject();
