...
public class SyncDataService {
    ...
    public void syncPlatformData(String accessToken, Long accountId, String startdate, String enddate) {
        try {
            ...
            noticeHelper.sendUserOperationNotification("数据同步","syncPlatformData", 2, "开始数据同步", String.valueOf(SecurityUtils.getUsername()));
            // 同步逻辑
            ...
            // 同步完成后写入 RuoYi 原生通知
            String detail = String.format("公众号ID：%s，时间范围：%s ~ %s，耗时：%d分%d秒",
                    accountId, startdate, enddate, minutes, remainSeconds);
            noticeHelper.sendUserOperationNotification("数据同步","syncPlatformData", 0, detail, String.valueOf(SecurityUtils.getUsername()));
        } catch (Exception e) {
            log.error("WebChatCaptureServiceImpl.syncPlatformData", e);
            // 同步失败也写入通知
            noticeHelper.sendUserOperationNotification("数据同步", "syncPlatformData", 1, e.getMessage(), String.valueOf(SecurityUtils.getUsername()));
        }
    }
