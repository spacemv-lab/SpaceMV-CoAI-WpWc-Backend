...
import com.txwx.social.crm.enums.ArticleStatusEnum;
import com.txwx.social.crm.mapper.TxwxArticleMapper;
...
import com.txwx.social.crm.util.Base64Util;
...
public class ArticleServiceImpl implements IArticleService {
    ...
    public void addDraft(ArticleVO articleVO) {
        try {
            ...
            String accessToken = buildAccessToken(articleVO.getAccountId());
            ...
            article.setAccountId(articleVO.getAccountId());
    ...
    @Override
    public List<TxwxArticlePO> getDraftList(ArticleDraftReqVO reqVO) {
        try {

            // 计算分页偏移量
            int offset = (reqVO.getPageNum() - 1) * reqVO.getPageSize();
            return txwxArticleMapper.selectDraftList(reqVO.getStatus(), reqVO.getSubmitter(), reqVO.getReviewer(), offset, reqVO.getPageSize(), reqVO.getAccountIds());
        } catch (Exception e) {
    ...
    @Override
    public int getDraftCount(ArticleDraftReqVO reqVO) {
        try {
            return txwxArticleMapper.selectDraftCount(reqVO.getStatus(), reqVO.getSubmitter(), reqVO.getReviewer(), reqVO.getAccountIds());
        } catch (Exception e) {
    ...
    public ArticleDetailVO getDraftDetail(Long id) {
        try {
            ...
            // 3. 转换为VO对象
            if (response.getNews_item() != null && !response.getNews_item().isEmpty()) {
                GetDraftDetailResponse.ArticleDetailItem item = response.getNews_item().get(0);
                com.txwx.social.crm.domain.vo.ArticleDetailVO detailVO = new com.txwx.social.crm.domain.vo.ArticleDetailVO();
    ...
    @Override
    public DraftListVO getDraftListFromTencent(Integer pageNum, Integer pageSize, Integer noContent, List<Long> accountIds) {
        try {
...
//            String accessToken = WebChatUtil.getAccessToken(appId, secret);
    ...
    @Override
    public PublishedArticleListVO getPublishedListFromTencent(Integer pageNum, Integer pageSize, Integer noContent, List<Long> accountIds) {
        try {
            // 将pageNum和pageSize转换为offset和count
            int offset = (pageNum - 1) * pageSize;
            int count = pageSize;
    ...
    @Override
    public List<TxwxArticlePO> getPublishedList(Integer pageNum, Integer pageSize, List<Long> accountIds) {
        try {
            // 计算分页偏移量
            int offset = (pageNum - 1) * pageSize;
            return txwxArticleMapper.selectPublishedList(offset, pageSize, accountIds);
        } catch (Exception e) {
    ...
    @Override
    public int getPublishedCount(List<Long> accountIds) {
        try {
            return txwxArticleMapper.selectPublishedCount(accountIds);
        } catch (Exception e) {
        ...
        try {
            ...
            for (TxwxArticlePO article : publishingArticles) {
                try {
                    ...
                    if (response.getPublish_status() != null) {
                        // 转换微信状态为本地状态
